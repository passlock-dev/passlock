import { type PublicKeyCredentialRequestOptionsJSON, type AuthenticationResponseJSON, startAuthentication as simpleAuthentication } from "@simplewebauthn/browser";
import { Micro, pipe } from "effect";
import { buildTenancyId, TenancyId } from "../../tenancy";
import { buildEndpoint, Endpoint, makeRequest, NetworkError } from "../../network";

export class AuthenticationError extends Micro.TaggedError("@error/AuthenticationError")<{
...
const fetchOptions = (userName?: string) => Micro.gen(function* () {
  ...
  return yield* makeRequest({ url, payload: { userName }, operation: 'options', responsePredicate: isOptionsResponse })
});
...
const isAuthenticationResponse = (payload: unknown): payload is AuthenticationResponse => {
  if (typeof payload !== 'object') return false;
  if (payload === null) return false;
...
const verifyCredential = (sessionToken: string, response: AuthenticationResponseJSON) => Micro.gen(function* () {
  const { endpoint } = yield* Micro.service(Endpoint)
  const { tenancyId } = yield* Micro.service(TenancyId);

  const url = new URL(`${tenancyId}/passkey/authentication/verification`, endpoint)

  return yield* makeRequest({ url, payload: { sessionToken, response }, operation: 'verification', responsePredicate: isAuthenticationResponse })
});

const startAuthentication = (optionsJSON: PublicKeyCredentialRequestOptionsJSON) => Micro.gen(function* () {
...
export const authenticatePasskey = (options: AuthenticationOptions): Micro.Micro<AuthenticationResponse, | NetworkError | AuthenticationError> => {
