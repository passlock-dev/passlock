...
const updatePasskeyUnsafe = updatePasskey as UnsafeUpdatePasskey
const deletePasskeyUnsafe = deletePasskey as UnsafeDeletePasskey

/**
 * Unsafe Passlock browser client.
 *
 * Methods resolve with success payloads or throw typed Passlock errors.
 *
 * @category Clients
 */
export class Passlock {
  ...
  registerPasskey(options: RegistrationOptions): Promise<RegistrationSuccess> {
    return registerPasskey(options, this.config)
  }

  authenticatePasskey(options: AuthenticationOptions): Promise<AuthenticationSuccess> {
    return authenticatePasskey(options, this.config)
  }

  updatePasskey(options: UpdatePasskeyOptions | UpdateCredentialOptions): Promise<UpdateSuccess> {
    return updatePasskeyUnsafe(options, this.config)
  }

  updatePasskeyUsernames(options: ReadonlyArray<UpdateCredentialOptions>): Promise<UpdateSuccess> {
  ...
  deletePasskey(
    ...
    return deletePasskeyUnsafe(options, this.config)
  }

  deleteUserPasskeys(options: ReadonlyArray<Credential>): Promise<DeleteSuccess> {
    return deleteUserPasskeys(options)
  }

  prunePasskeys(options: PrunePasskeyOptions): Promise<PruningSuccess> {
