...
import { PUBLIC_PASSLOCK_TENANCY_ID as tenancyId, PUBLIC_PASSLOCK_ENDPOINT as endpoint } from "$env/static/public";
...
const findAllPasskeys = async () => {
  const passkeys = await db.selectDistinct({ passkeyId: passkeysTable.passkeyId }).from(passkeysTable);
