  ...
  onMount(() => {
    google.accounts.id.initialize({
      ...
      callback: async ({ credential }) => { 
        const principal = operation === 'register' ? 
          await passlock.registerOidc({ provider: 'google', idToken: credential }) : 
          await passlock.authenticateOidc({ provider: 'google', idToken: credential })
    ...
    // Uncomment to enable one tap prompt
...
{#if error}
  <div class="mt-2 text-sm text-center text-red-600 dark:text-red-400">{error}</div>
{/if}

{#if error}
  <div class="text-sm text-center text-red-600 dark:text-red-400">{errorDetail}</div>
{/if}
