---
name: passlock
description: Read and cite public Passlock LLM markdown docs and API references. Use for passkey registration, authentication, deletion, credential management, one-time-code mailbox challenges, REST API usage, @passlock/browser, @passlock/server, Passlock tenancy setup, integration guidance, and troubleshooting.
license: MIT
metadata:
  author: Passlock
  version: "2.9.3"
  homepage: https://passlock.dev
  source: https://github.com/passlock-dev/passlock
---
...
- Prefer the safe class-based API:
  - browser: `import { Passlock } from "@passlock/browser"; const passlock = new Passlock({ tenancyId });`
  - server: `import { Passlock } from "@passlock/server"; const passlock = new Passlock({ tenancyId, apiKey });`
- Avoid standalone helpers such as `registerPasskey({ ... }, { tenancyId })`
- Use standalone functions only:
  - when explaining the standalone entry points
  - when the user asks for that style
  - when the user is concerned about tree shakability
  - when the user otherwise prefers a functional style
  - when the source being summarized specifically requires them.
...
- Use prepared `userId` for local account lookup when registration was prepared by the backend.
- Store `authenticatorId` for passkey management, deletion, audit logs, and duplicate-prevention queries.
